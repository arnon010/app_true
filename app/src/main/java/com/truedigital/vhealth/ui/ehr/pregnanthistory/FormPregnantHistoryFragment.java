...
public class FormPregnantHistoryFragment extends BaseMvpFragment<FormPregnantHistoryFragmentInterface.Presenter>
        implements FormPregnantHistoryFragmentInterface.View {
    ...
    public FormPregnantHistoryFragment() {
        super();
    }

    public static FormPregnantHistoryFragment newInstance(PregnantHistoryObject data, boolean isNew, boolean isNewFromMenu) {
        FormPregnantHistoryFragment fragment = new FormPregnantHistoryFragment();
        fragment.data = data;
    ...
    @Override
    public FormPregnantHistoryFragmentInterface.Presenter createPresenter() {
        return FormPregnantHistoryFragmentPresenter.create();
    }

    @Override
    public int getLayoutView() {
        return R.layout.fragment_form_pregnant_history;
    }
    ...
    public void initialize() {
        ...
        if (!this.isNew) {
            ((MainActivity) getActivity()).updateToolbar(getResources().getString(R.string.update_pregnant_history), false, null, false);

            edtTitle.setText(data.getTitle());
            edtDate.setText(ConvertDate.convertDateSimpleShow(data.getRecordDate()));
            edtPlace.setText(data.getPlace());
            edtDescription.setText(data.getDetail());
            btnSave.setText(getResources().getString(R.string.update_pregnant_history));
            btnSave.setOnClickListener(this.onUpdateClickListener());
        } else {
            ((MainActivity) getActivity()).updateToolbar(getResources().getString(R.string.new_pregnant_history), false, null, false);

            btnSave.setText(getResources().getString(R.string.new_pregnant_history));
        ...
        edtDate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                selectStartDateBottomSheet = CalendarBottomSheetFragment.newInstance(getActivity(), (int) CommonUtils.convertDpToPixel(getActivity(), 370), data.getRecordDate());
        ...
        data.getAttachmentList().add(new PregnantHistoryImageObject());
        setTempImageList();
    ...
    private void setTempImageList() {
        ...
        ArrayList<PregnantHistoryImageObject> temp = new ArrayList<>();
        for (PregnantHistoryImageObject img : data.getAttachmentList()) {
    ...
    private ListImageAdapter.OnClickListener<PregnantHistoryImageObject> onImageListClickListener() {
        return new ListImageAdapter.OnClickListener<PregnantHistoryImageObject>() {
            @Override
            ...
            @Override
            public void onViewClick(PregnantHistoryImageObject data) {
                if (!data.getIsVideo()) {
            ...
            @Override
            public void onDelete(final PregnantHistoryImageObject img) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent dataIntent) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            PregnantHistoryImageObject imgNew = new PregnantHistoryImageObject();
    ...
    private void setData() {
        ...
        data.setPlace(edtPlace.getText().toString());
    ...
    private void checkRequire() {
        if (edtTitle.getText().toString() == null
                ...
                || edtPlace.getText().toString() == null
                || edtPlace.getText().toString().equals("")) {
    ...
    private View.OnClickListener onNewClickListener() {
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                setData();
                getPresenter().newPregnantHistory(getActivity(), data);
    ...
    private View.OnClickListener onUpdateClickListener() {
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                setData();
                getPresenter().updatePregnantHistory(getActivity(), data);
    ...
    @Override
    public void onNewSuccess(final PregnantHistoryObject data) {
        SuccessDialog dialog = new SuccessDialog(getActivity());
        dialog.showDialog(getResources().getString(R.string.new_pregnant_history_success));
        dialog.setOnShowFinishListener(new SuccessDialog.OnShowFinishListener() {
            @Override
            public void onShowFinish() {
                MainActivity main = ((MainActivity) getActivity());
                if (isNewFromMenu) {
                    main.removeToFragment(FormPregnantHistoryFragment.class.toString());
                    main.openPregnantHistory(data.getPatientId(), data.getPatientMenuId());
                } else {
                    int index = getActivity().getSupportFragmentManager().getBackStackEntryCount() - 2;
                    if (index >= 0) {
                        FragmentManager.BackStackEntry backEntry = getFragmentManager().getBackStackEntryAt(index);
                        String tag = backEntry.getName();
                        PregnantHistoryFragment fragment = (PregnantHistoryFragment) getFragmentManager().findFragmentByTag(tag);
                        fragment.clearList();
                    }
    ...
    @Override
    public void onUpdateSuccess(PregnantHistoryObject data) {
        SuccessDialog dialog = new SuccessDialog(getActivity());
        dialog.showDialog(getResources().getString(R.string.update_pregnant_history_success));
        dialog.setOnShowFinishListener(new SuccessDialog.OnShowFinishListener() {
            @Override
            public void onShowFinish() {
                int index = getActivity().getSupportFragmentManager().getBackStackEntryCount() - 2;
