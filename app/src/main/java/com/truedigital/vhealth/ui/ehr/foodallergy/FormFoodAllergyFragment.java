...
public class FormFoodAllergyFragment extends BaseMvpFragment<FormFoodAllergyFragmentInterface.Presenter>
        implements FormFoodAllergyFragmentInterface.View {
    ...
    public FormFoodAllergyFragment() {
        super();
    }

    public static FormFoodAllergyFragment newInstance(FoodAllergyObject data, boolean isNew, boolean isNewFromMenu) {
        FormFoodAllergyFragment fragment = new FormFoodAllergyFragment();
        fragment.data = data;
    ...
    @Override
    public FormFoodAllergyFragmentInterface.Presenter createPresenter() {
        return FormFoodAllergyFragmentPresenter.create();
    }

    @Override
    public int getLayoutView() {
        return R.layout.fragment_form_food_allergy;
    }
    ...
    public void initialize() {
        ...
        if (!this.isNew) {
            ((MainActivity) getActivity()).updateToolbar(getResources().getString(R.string.update_food_allergy), false,null, false);

            edtTitle.setText(data.getTitle());
            edtDescription.setText(data.getDetail());
            btnSave.setText(getResources().getString(R.string.update_food_allergy));
            btnSave.setOnClickListener(this.onUpdateClickListener());
        } else {
            ((MainActivity) getActivity()).updateToolbar(getResources().getString(R.string.new_food_allergy), false,null, false);

            btnSave.setText(getResources().getString(R.string.new_food_allergy));
        ...
        data.getAttachmentList().add(new FoodAllergyImageObject());
        setTempImageList();
    ...
    private void setTempImageList() {
        ...
        ArrayList<FoodAllergyImageObject> temp = new ArrayList<>();
        for (FoodAllergyImageObject img : data.getAttachmentList()) {
    ...
    private ListImageAdapter.OnClickListener<FoodAllergyImageObject> onImageListClickListener() {
        return new ListImageAdapter.OnClickListener<FoodAllergyImageObject>() {
            @Override
            ...
            @Override
            public void onViewClick(FoodAllergyImageObject data) {
                if (!data.getIsVideo()) {
            ...
            @Override
            public void onDelete(final FoodAllergyImageObject img) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent dataIntent) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            FoodAllergyImageObject imgNew = new FoodAllergyImageObject();
    ...
    private View.OnClickListener onNewClickListener() {
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                setData();
                getPresenter().newFoodAllergy(getActivity(), data);
            }
        };
    }
    ...
    private View.OnClickListener onUpdateClickListener() {
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                setData();
                getPresenter().updateFoodAllergy(getActivity(), data);
            }
        };
    }
    ...
    @Override
    public void onNewSuccess(final FoodAllergyObject data) {
        SuccessDialog dialog = new SuccessDialog(getActivity());
        dialog.showDialog(getResources().getString(R.string.new_food_allergy_success));
        dialog.setOnShowFinishListener(new SuccessDialog.OnShowFinishListener() {
            @Override
            public void onShowFinish() {
                MainActivity main = ((MainActivity) getActivity());
                if (isNewFromMenu) {
                    main.removeToFragment(FormFoodAllergyFragment.class.toString());
                    main.openFoodAllergy(data.getPatientId(), data.getPatientMenuId());
                } else {
                    int index = getActivity().getSupportFragmentManager().getBackStackEntryCount() - 2;
                    if (index >= 0) {
                        FragmentManager.BackStackEntry backEntry = getFragmentManager().getBackStackEntryAt(index);
                        String tag = backEntry.getName();
                        FoodAllergyFragment fragment = (FoodAllergyFragment) getFragmentManager().findFragmentByTag(tag);
    ...
    @Override
    public void onUpdateSuccess(FoodAllergyObject data) {
        SuccessDialog dialog = new SuccessDialog(getActivity());
        dialog.showDialog(getResources().getString(R.string.update_food_allergy_success));
        dialog.setOnShowFinishListener(new SuccessDialog.OnShowFinishListener() {
            @Override
            public void onShowFinish() {
                int index = getActivity().getSupportFragmentManager().getBackStackEntryCount() - 2;
