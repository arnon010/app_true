...
public class FormMedicationHistoryFragment extends BaseMvpFragment<FormMedicationHistoryFragmentInterface.Presenter>
        implements FormMedicationHistoryFragmentInterface.View {
    ...
    public FormMedicationHistoryFragment() {
        super();
    }

    public static FormMedicationHistoryFragment newInstance(MedicationHistoryObject data, boolean isNew, boolean isNewFromMenu) {
        FormMedicationHistoryFragment fragment = new FormMedicationHistoryFragment();
        fragment.data = data;
    ...
    @Override
    public FormMedicationHistoryFragmentInterface.Presenter createPresenter() {
        return FormMedicationHistoryFragmentPresenter.create();
    }

    @Override
    public int getLayoutView() {
        return R.layout.fragment_form_medication_history;
    }
    ...
    public void initialize() {
        ...
        if (!this.isNew) {
            ((MainActivity) getActivity()).updateToolbar(getResources().getString(R.string.update_medication_history), false,null, false);
            ...
            btnSave.setText(getResources().getString(R.string.update_medication_history));
            btnSave.setOnClickListener(this.onUpdateClickListener());
        } else {
            ((MainActivity) getActivity()).updateToolbar(getResources().getString(R.string.new_medication_history), false,null, false);

            btnSave.setText(getResources().getString(R.string.new_medication_history));
        ...
        data.getAttachmentList().add(new MedicationHistoryImageObject());
        setTempImageList();
    ...
    private void setTempImageList() {
        ...
        ArrayList<MedicationHistoryImageObject> temp = new ArrayList<>();
        for (MedicationHistoryImageObject img : data.getAttachmentList()) {
    ...
    private ListImageAdapter.OnClickListener<MedicationHistoryImageObject> onImageListClickListener() {
        return new ListImageAdapter.OnClickListener<MedicationHistoryImageObject>() {
            ...
            @Override
            public void onDelete(final MedicationHistoryImageObject img) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent dataIntent) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            MedicationHistoryImageObject imgNew = new MedicationHistoryImageObject();
    ...
    private View.OnClickListener onNewClickListener() {
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                setData();
                getPresenter().newMedicationHistory(getActivity(), data);
            }
        };
    }
    ...
    private View.OnClickListener onUpdateClickListener() {
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                setData();
                getPresenter().updateMedicationHistory(getActivity(), data);
            }
        };
    }
    ...
    @Override
    public void onNewSuccess(final MedicationHistoryObject data) {
        SuccessDialog dialog = new SuccessDialog(getActivity());
        dialog.showDialog(getResources().getString(R.string.new_medication_history_success));
        dialog.setOnShowFinishListener(new SuccessDialog.OnShowFinishListener() {
            @Override
            public void onShowFinish() {
                MainActivity main = ((MainActivity) getActivity());
                if (isNewFromMenu) {
                    main.removeToFragment(FormMedicationHistoryFragment.class.toString());
                    main.openMedicationHistory(data.getPatientId(), data.getPatientMenuId());
                } else {
                    int index = getActivity().getSupportFragmentManager().getBackStackEntryCount() - 2;
                    if (index >= 0) {
                        FragmentManager.BackStackEntry backEntry = getFragmentManager().getBackStackEntryAt(index);
                        String tag = backEntry.getName();
                        MedicationHistoryFragment fragment = (MedicationHistoryFragment) getFragmentManager().findFragmentByTag(tag);
                        fragment.clearList();
                    }
    ...
    @Override
    public void onUpdateSuccess(MedicationHistoryObject data) {
        SuccessDialog dialog = new SuccessDialog(getActivity());
        dialog.showDialog(getResources().getString(R.string.update_medication_history_success));
        dialog.setOnShowFinishListener(new SuccessDialog.OnShowFinishListener() {
            @Override
            public void onShowFinish() {
                int index = getActivity().getSupportFragmentManager().getBackStackEntryCount() - 2;
