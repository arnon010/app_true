...
public class ArticlesDetailFragmentPresenter extends BaseMvpPresenter<ArticlesDetailFragmentInterface.View>
        implements ArticlesDetailFragmentInterface.Presenter{

    private List<ItemArticleGroupDao> listData;

    public static ArticlesDetailFragmentInterface.Presenter create(){
        return new ArticlesDetailFragmentPresenter();
    }
    ...
    /*
    private void callGetListAppointments(int appointment_type) {
        getView().showLoading();

        Call<ListAppointmentDao> call = getRetrofitToken(AppManager.getDataManager().getAccess_token()).getListAppointments();
