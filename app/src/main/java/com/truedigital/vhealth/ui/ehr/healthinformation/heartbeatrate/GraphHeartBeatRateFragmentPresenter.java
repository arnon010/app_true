...
public class GraphHeartBeatRateFragmentPresenter extends BaseMvpPresenter<GraphHeartBeatRateFragmentInterface.View>
        implements GraphHeartBeatRateFragmentInterface.Presenter{

    public static GraphHeartBeatRateFragmentInterface.Presenter create(){
        return new GraphHeartBeatRateFragmentPresenter();
    }

    @Override
    public void getGraphHeartBeatRate(GraphHeartBeatRateCriteriaObject criteria) {
        getView().showLoading();

        String access_token = AppManager.getDataManager().getAccess_token();

        Call<ApiChartObject> call = getRetrofitToken(access_token).getChartDataHeartRate(criteria.getPatientId(), criteria.getPatientMenuId(), criteria.getType(), criteria.getPeriod());
        call.enqueue(new Callback<ApiChartObject>() {
            ...
            public void onResponse(Call<ApiChartObject> call, Response<ApiChartObject> response) {
                if (response.isSuccessful() && response.body().isSuccess()) {
                    ...
                    getView().setGraphHeartBeatRate(data);
